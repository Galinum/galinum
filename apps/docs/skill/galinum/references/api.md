...
## Launch activation (Galinum Cloud)

These four operations are Cloud-only; self-host returns `501`. GET accepts a
customer project secret or hosted-agent key. PATCH accepts only a customer
project secret; a hosted key receives `403`. Approval remains the human
manager-session action. These endpoints never grant approval or deployment
configuration authority. A human manager selects the repository, exact production
environment, associated sources, and explicitly confirms what that deployment ships.
...
activation view in the same shape as GET. These are separate Cloud controls,
not fields on common campaign create, update, or read payloads.
...
expiry still block activation. Future `deliverFrom` delays delivery but does
