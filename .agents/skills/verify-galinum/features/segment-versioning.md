...
- Read history through `/api/v1/segments/{id}/versions` and `/api/v1/segments/{id}/versions/{version}`.
...
- **Create.** Step 1 creates a free-plan segment. Status `201` returns an active segment at version 1.
- **Revise.** Step 2 replaces the expression with enterprise-plan matching and `expectedVersion: 1`. The active segment advances to version 2 with the new expression.
