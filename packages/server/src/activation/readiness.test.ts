...
describe("shared current campaign definition readiness", () => {
  ...
  it.each(["/news", "mailto:support@example.test", "https://example.test/news", "http://example.test/news"])("rejects legacy web CTA URL %s", async (url) => {
    expect((await check(definition({ body: "Body", cta: { label: "Read", url } }))).ok).toBe(false);
  });
  it.each([{ kind: "website", url: "https://example.test/news" }, { kind: "app", url: "example://news" }])("accepts typed CTA %j", async (destination) => {
    expect(await check(definition({ body: "Body", cta: { label: "Read", destination } }))).toEqual({ ok: true });
  });
  it.each([
