...
export class PublicSqlActivationData implements ProductActivationData {
  constructor(private readonly executor: Kysely<ProductDB>, private readonly projectId: string) {
  ...
  async saveMapping(value: ActivationMapping): Promise<void> {
    if (!this.executor.isTransaction) {
      await this.executor.transaction().execute(transaction => new PublicSqlActivationData(transaction, this.projectId).saveMapping(value));
      return;
    }
    await sql`select pg_advisory_xact_lock(hashtextextended(${this.projectId}, 0))`.execute(this.executor);
  ...
  async sources(): Promise<StockSource[]> {
    const rows = await this.executor.selectFrom("shipping_sources").selectAll().where("project_id", "=", this.projectId).orderBy("id").execute();
    return rows.map(row => ({ id: row.id, installationId: Number(row.installation_id), repositoryId: Number(row.repo_id), ...repository(row.repo_name),
  ...
  async saveSource(value: StockSource): Promise<void> {
    ...
    const row = await this.executor.insertInto("shipping_sources").values({ id: value.id, project_id: this.projectId, ...fields })
      .onConflict(conflict => conflict.column("id").doUpdateSet(fields).where("shipping_sources.project_id", "=", this.projectId)).returning("id").executeTakeFirst();
    if (!row) throw new Error("Source belongs to another project");
  }

  async controls(): Promise<StockProjectControls> {
    const row = await this.executor.selectFrom("shipping_project_controls").selectAll().where("project_id", "=", this.projectId).executeTakeFirst();
  ...
  async saveControls(value: StockProjectControls): Promise<void> {
    ...
    await this.executor.insertInto("shipping_project_controls").values({ project_id: this.projectId, ...fields })
      .onConflict(conflict => conflict.column("project_id").doUpdateSet(fields)).execute();
  }
