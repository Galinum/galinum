...
const allowed = new Set(["type", "properties", "required", "additionalProperties", "items", "enum", "$ref", "minimum", "maximum", "minLength", "maxLength", "pattern", "maxItems", "uniqueItems", "description", "example"]);
export function schemaType(schema, schemas) {
  ...
  if (schema.$ref) {
    if (Object.keys(schema).some((key) => !["$ref", "description", "example"].includes(key))) throw new Error("Mobile references cannot have constraint siblings");
...
export function generateMobile(contract) {
  ...
  const schemas = Object.fromEntries(Object.entries(contract.components.schemas).filter(([name]) => name.startsWith("Installation")));
