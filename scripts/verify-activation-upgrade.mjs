...
async function rows(client, tables) {
  const result = {};
  for (const table of tables) result[table] = (await client.query(`SELECT to_jsonb(t)::text AS row FROM public.${quote(table)} t ORDER BY to_jsonb(t)::text COLLATE "C"`)).rows.map((row) => row.row);
  return result;
}
...
try {
  ...
  const before=await rows(upgrade,oldTables);
  ...
  assert.deepEqual(await rows(upgrade,oldTables),before,"Upgrade changed baseline rows");
